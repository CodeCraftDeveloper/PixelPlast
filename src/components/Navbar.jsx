...
function Navbar() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 w-full bg-[#fff] px-3 pt-3 sm:px-4">
      <nav className="flex items-center justify-between rounded-[1.25rem] bg-[#fff] px-5 py-3">
        <a
          href="#home"
        ...
        <ul className="hidden items-center gap-7 text-xs text-[#6b7280] md:flex">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
        ...
        <menu className="m-0 flex list-none items-center gap-2 p-0">
          <a
            href="#contact"
            className="rounded-full bg-[#31d3a2] px-3.5 py-1.5 text-xs font-medium text-white"
          >
            Get in touch
          </a>
        </menu>
